            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Edit Admin Data</li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="col-xxl-12 d-flex align-items-stretch">
                <div class="card w-100 overflow-hidden rounded-4">
                    <div class="card-body position-relative p-4">
                        <div class="row ">
                            <form method="post" action="{{ route('admin.manage_admin.update', $admin->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat" rows="3" required>{{ old('alamat', $admin->alamat) }}</textarea>
                                    @error('alamat')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                                        ...
                                        @if(isset($roles))
                                            @foreach($roles as $role)
                                                <option value="{{ $role->id }}" {{ old('role', $admin->role) == $role->id ? 'selected' : '' }}>{{ $role->nama_role }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('role')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('wilayah') is-invalid @enderror" id="wilayah" name="wilayah" required>
                                        <option value="">Pilih Wilayah</option>
                                        @if(isset($wilayahs))
                                            @foreach($wilayahs as $wilayah)
                                                <option value="{{ $wilayah->id }}" {{ old('wilayah', $admin->wilayah) == $wilayah->id ? 'selected' : '' }}>{{ $wilayah->namaWilayah }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('wilayah')
