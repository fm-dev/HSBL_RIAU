   <!--start sidebar-->
   <aside class="sidebar-wrapper" data-simplebar="true">
    <div class="sidebar-header">
      <div class="logo-icon">
        <img src="{{ asset('clientSide/images/logo/logo.png')}}" class="logo-img" alt="">
      </div>
      <div class="logo-name flex-grow-1">
        <h5 class="mb-0">HSLBL Admin</h5>
      </div>
      <div class="sidebar-close">
        <span class="material-icons-outlined">close</span>
      </div>
    </div>
    <div class="sidebar-nav">
        <!--navigation-->
        <ul class="metismenu" id="sidenav">
          @if(isset($menus) && !empty($menus))
            
            @foreach($menus as $menu)
              <li>
                <a href="javascript:;" class="has-arrow">
                  <div class="parent-icon"><i class="material-icons-outlined">home</i></div>
                  <div class="menu-title">{{ $menu['labelMenu'] }}</div>
                </a>
                @if(!empty($menu['children']))
                  <ul>
                    @foreach($menu['children'] as $child)
                      <li>
                        <a href="{{ $child['path'] }}">
                          <i class="material-icons-outlined">arrow_right</i>{{ $child['labelMenu'] }}
                        </a>
                      </li>
                    @endforeach
                  </ul>
                @endif
              </li>
            @endforeach
          @else
            <!-- Fallback menu jika data tidak ada -->
            <!-- <li>
              <a href="javascript:;" class="has-arrow">
                <div class="parent-icon"><i class="material-icons-outlined">home</i>
                </div>
                <div class="menu-title">Master Data</div>
              </a>
              <ul>
                <li><a href="\admin\manage_admin"><i class="material-icons-outlined">arrow_right</i>Manage User Admin</a>
                </li>
                <li><a href="index2.html"><i class="material-icons-outlined">arrow_right</i>eCommerce</a>
                </li>
              </ul>
            </li>
            <li>
              <a href="javascript:;" class="has-arrow">
                <div class="parent-icon"><i class="material-icons-outlined">widgets</i>
                </div>
                <div class="menu-title">Admin</div>
              </a>
              <ul>
                <li><a href="/portal/admin/profile"><i class="material-icons-outlined">arrow_right</i>Profile Admin</a>
                </li>
                <li><a href="/portal/admin/dataAdmin"><i class="material-icons-outlined">arrow_right</i>Data Admin</a>
                </li>
              </ul>
            </li>
            <li>
              <a href="javascript:;" class="has-arrow">
                <div class="parent-icon"><i class="material-icons-outlined">widgets</i>
                </div>
                <div class="menu-title">User</div>
              </a>
              <ul>
                <li><a href="/portal/admin/useralldata"><i class="material-icons-outlined">arrow_right</i>Data users</a>
                </li>
              </ul>
            </li>
            <li>
              <a class="has-arrow" href="javascript:;">
                <div class="parent-icon"><i class="material-icons-outlined">apps</i>
                </div>
                <div class="menu-title">Kompetisi</div>
              </a>
              <ul>
                <li><a href="/portal/admin/team/list"><i class="material-icons-outlined">arrow_right</i>Team List</a>
                </li>
              </ul>
            </li>
            <li>
              <a href="javascript:;" class="has-arrow">
                <div class="parent-icon"><i class="material-icons-outlined">shopping_bag</i>
                </div>
                <div class="menu-title">Events</div>
              </a>
              <ul>
                <li><a href="/portal/admin/events/listseasons"><i class="material-icons-outlined">arrow_right</i>Seasons</a>
                </li>
                <li><a href="/portal/admin/events/listkompetisi"><i class="material-icons-outlined">arrow_right</i>Kompetisi</a>
                </li>
                <li><a href="/portal/admin/events/listSeries"><i class="material-icons-outlined">arrow_right</i>Series</a>
                </li>
                <li><a href="/portal/admin/events/listSekolah"><i class="material-icons-outlined">arrow_right</i>Sekolah</a>
                </li>
              </ul>     
            </li>
            <li>
              <a class="has-arrow" href="javascript:;">
                <div class="parent-icon"><i class="material-icons-outlined">card_giftcard</i>
                </div>
                <div class="menu-title">Events Score</div>
              </a>
              <ul>
                <li><a href="/portal/admin/events/listScore"><i class="material-icons-outlined">arrow_right</i>Histori Pertandingan</a>
                </li>
              </ul>
            </li> -->
          @endif
         </ul>
        <!--end navigation-->
    </div>
  </aside>
<!--end sidebar-->